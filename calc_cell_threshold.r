...
option_list = list(
    ...
    make_option(c("-b", "--log_base"), type="integer", default=10,
                help="lower reads threshold for barcodes to calculate mix gaussian [default=%default]", metavar="number"),
    make_option(c("-l", "--lower_threshold"), type="integer", default=100,
                help="lower reads threshold for barcodes to calculate mix gaussian [default=%default]", metavar="number"),
    make_option(c("-t", "--prob_threshold"),  default=.1,
                help="threshold of probability to filter in last Gaussian population [default=%default]", metavar="double")
);


opt = parse_args( OptionParser(option_list=option_list))
sample <-  opt$sample # "PDL1"
lower.th <- opt$lower_threshold
th.prob <-  opt$prob_threshold
log.base <- opt$log_base
