...
def qc_metrics(args):
        ...
        with gzip.open(tagalign_file, 'wt') as f:
            ...
            for read in bamfile:
                ...
                if read.is_reverse:
                    ...
                    read_orient = '+'
                line_out = '\t'.join([read_chr, str(read_start), str(read_end), '{}_{}'.format(
        ...
        for read in bamfile:
            ...
            if not read.is_duplicate:
                if read.reference_name in chr_names:
                    qc_metrics[barcode]['unique_usable_reads'] = qc_metrics[barcode].get(
                        'unique_usable_reads', 0) + 1
                elif read.reference_name == 'chrM':
                    qc_metrics[barcode]['unique_mito_reads'] = qc_metrics[barcode].get(
                        'unique_mito_reads', 0) + 1
                qc_metrics[barcode]['total_sequenced_reads'] = qc_metrics[barcode].get(
                    'total_sequenced_reads', 0) + 1
            else:
                qc_metrics[barcode]['duplicated_reads'] = qc_metrics[barcode].get(
                    'duplicated_reads', 0) + 1
                qc_metrics[barcode]['total_sequenced_reads'] = qc_metrics[barcode].get(
                    'total_sequenced_reads', 0) + 1
    else:
    ...
    tss_cmd = subprocess.Popen(['bedtools', 'intersect', '-a', tagalign_file,
                                '-b', args.promoter_file, '-wa', '-wb'], stdout=subprocess.PIPE)
    ...
    qc_metrics['frac_reads_in_promoters'] = qc_metrics['reads_in_promoters'].div(
        qc_metrics['unique_usable_reads']).replace(np.inf, 0).fillna(0)
    qc_metrics['frac_promoters_used'] = qc_metrics['tss_used'] / total_prom
    qc_metrics['frac_mito_reads'] = qc_metrics['unique_mito_reads'].div(
...
def generate_matrix(args):
    ...
    awk = subprocess.Popen(
        ['awk', '''BEGIN{{OFS="\\t"}} {{print $2,$3,$1}}'''], stdin=uniq.stdout, stdout=subprocess.PIPE)
    ...
    subprocess.call(['gzip', lf_mtx_file])
...
def intersect_regions(tagalign, regions):
    awk_cmd = [
        'awk', '''BEGIN{{FS=OFS="\\t"}} {{peakid=$1":"$2"-"$3; gsub("chr","",peakid); print $1, $2, $3, peakid}}''', regions]
    intersect_cmd = ['bedtools', 'intersect',
                     '-a', tagalign, '-b', '-', '-wa', '-wb']
    awk = subprocess.Popen(awk_cmd, stdout=subprocess.PIPE)
    intersect = subprocess.Popen(
        intersect_cmd, stdin=awk.stdout, stdout=subprocess.PIPE)
    return intersect


def make_windows(args):
    makewindows_cmd = ['bedtools', 'makewindows', '-g',
                       args.chrom_sizes, '-w', str(args.window_size * 1000)]
    filter_cmd = ['grep', '-v', '_']
    blacklist_cmd = ['bedtools', 'intersect',
                     '-a', '-', '-b', args.blacklist_file, '-v']
    windows_file = '{}.{}kb_windows.bed'.format(
        args.output_prefix, args.window_size)
    with open(windows_file, 'w') as f:
        makewindows = subprocess.Popen(makewindows_cmd, stdout=subprocess.PIPE)
        filt = subprocess.Popen(
            filter_cmd, stdin=makewindows.stdout, stdout=subprocess.PIPE)
...
def main(args):
    ...
    if not args.skip_rmdup:
        logging.info('Removing duplicate and mitochrondrial reads.'.format(
            args.minimum_reads))
...
def process_args():
    parser = argparse.ArgumentParser(
        description='Use 10X output to process snATAC-seq data.')
    io_group = parser.add_argument_group('I/O arguments')
    io_group.add_argument('-b', '--input-bam', required=True,
                          type=str, help='Position sorted bam from 10X output')
    io_group.add_argument('-o', '--output', required=False, type=str,
                          default=os.getcwd(), help='Output directory to store processed files')
    io_group.add_argument('-n', '--name', required=True,
                          type=str, help='Prefix for naming all output files')

    align_group = parser.add_argument_group('Alignment arguments')
    align_group.add_argument('-t', '--threads', required=False, type=int,
                             default=8, help='Number of threads to use for alignment [8]')
    align_group.add_argument('-m', '--memory', required=False, type=int, default=4,
                             help='Maximum amount of memory (G) per thread for samtools sort [4]')
    align_group.add_argument('-q', '--map-quality', required=False, type=int,
                             default=30, help='Mapping quality score filter for samtools [30]')
    align_group.add_argument('-ref', '--reference', required=False, type=str,
                             default='/home/joshchiou/references/male.hg19.fa', help='Path to the reference genome')

    dup_group = parser.add_argument_group('Remove duplicates arguments')
    dup_group.add_argument('--picard', required=False, type=str,
                           default='/home/joshchiou/bin/picard.jar', help='Path to picard.jar')

    matrix_group = parser.add_argument_group('Matrix generation arguments')
    matrix_group.add_argument(
        '--shift', required=False, type=int, default=-100, help='Read shift length')
    matrix_group.add_argument(
    ...
    matrix_group.add_argument('--window-size', required=False, type=int,
                              default=5, help='Size (kb) to use for defining windows of accessibility')
    matrix_group.add_argument('--chrom-sizes', required=False, type=str,
                              default='/home/joshchiou/references/hg19.chrom.sizes', help='Chromosome sizes file from UCSC')
    matrix_group.add_argument('--blacklist-file', required=False, type=str,
                              default='/home/joshchiou/references/ENCODE.hg19.blacklist.bed', help='BED file of blacklisted regions')
    matrix_group.add_argument('--promoter-file', required=False, type=str,
                              default='/home/joshchiou/references/gencode.v19.2kb_autosomal_prom_uniq.bed', help='BED file of autosomal promoter regions')
    matrix_group.add_argument(
        '--peak-file', required=False, type=str, help='Narrow peak file')
    matrix_group.add_argument('-g', '--genome', required=False, type=str,
                              default='mm', help='Genome in macs2 peak call mm or hs')

    skip_group = parser.add_argument_group('Skip steps')
    skip_group.add_argument('--skip-convert', required=False,
                            action='store_true', default=False, help='Skip bam conversion step')
    skip_group.add_argument('--skip-rmdup', required=False, action='store_true',
                            default=False, help='Skip duplicate removal step')
    skip_group.add_argument('--skip-qc', required=False, action='store_true',
                            default=False, help='Skip quality metrics step')
    skip_group.add_argument('--skip-matrix', required=False, action='store_true',
                            default=False, help='Skip matrix generation step')
    return parser.parse_args()


if __name__ == '__main__':
    logging.basicConfig(
        format='[%(filename)s] %(asctime)s %(levelname)s: %(message)s', datefmt='%I:%M:%S', level=logging.INFO)
    args = process_args()
    main(args)
